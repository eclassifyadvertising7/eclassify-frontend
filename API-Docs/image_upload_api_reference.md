...
async  | Boolean | Whether to perform the archive generation in the background (asynchronously). **Applicable for:****SDKs**: `create_zip` and `create_archive` methods only**REST API**: `create` mode only**Default**: `false`.
